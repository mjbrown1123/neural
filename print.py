...
#this section saves the weight values for each synapse in a text file
text = open(synapse_file, "w")

syn0_list = syn0.tolist()
syn1_list = syn1.tolist()
syn2_list = syn2.tolist()

rows, columns = syn0.shape

string = "syn0 (" + str(rows) + "x" + str(columns) + "): \n"
text.write(string)

for iter in range(rows):
    for j in range(columns):

        text.write(str(syn0_list[iter][j]) + ",")

text.write("\n\n")


rows, columns = syn1.shape

string = "syn1 (" + str(rows) + "x" + str(columns) + "): \n"
text.write(string)

for iter in range(rows):
    for j in range(columns):

        text.write(str(syn1_list[iter][j]) + ",")

text.write("\n\n")


rows, columns = syn2.shape

string = "syn2 (" + str(rows) + "x" + str(columns) + "): \n"
text.write(string)

for iter in range(rows):
    for j in range(columns):

        text.write(str(syn2_list[iter][j]) + ",")
